...
An operator-provided Host tree that combines an **Artifact** with Propraetor-internal **Workload Manifest** and **Binding**, declares **Intent**, and may author **Declarations** for Components. Identified by the basename of its Environment directory (immediate non-hidden child of the Environment); **Mirror** materializes it onto Host Volume `workloads/<basename>/` regardless of Source, with optional nested **Persist**; consumers read known siblings selectively. Canonical Service Network hostname is that basename (except the reserved dial name `database`). Does not gather peers or fulfill other Workloads’ Declarations; not Fabric; not a Component; never installed during Initial Host Provisioning. **Workload** is canonical in docs and code; **workflow** is an accepted conversational alias.
