...
**Declaration**:
A statement of what a Workload **Provides** or **Requires**, or an Environment-authored claim (today: Domain assignment feeding Edge). Binding may resolve Workload Declarations against Environment Domains and Environment Configuration; Components may gather the subset they fulfill (bound Routes → Edge; Requires `database` → Database; Domain assignment → Edge). Distinct from Workload Intent, from **Binding** (Propraetor-internal resolution), and from fulfillment artifacts inside Component interior.
...
**Domain front**:
Edge-owned per-FQDN drop-in for one want-list name: the HTTPS `server` that terminates TLS for that Domain name and includes matching bound Workload Routes. Publishes Edge baseline `/healthcheck` and the per-name `:80`→HTTPS redirect without a Workload Route. Shape SoT is Edge Component `domain-template.conf`; Edge Setup renders each want-list FQDN into data `domains/<fqdn>.conf` (not Workload-authored Route SoT). Never Workload-owned and never ACME-mutated.
_Avoid_: Domain Route, Edge Route, vhost (when you mean this Edge-owned front); Workload Route

**Workload** (alias **workflow**):
An operator-provided Host tree that combines an **Artifact** with Propraetor-internal **Workload Manifest** and **Binding**, declares **Intent**, and may author **Declarations** for Components. Identified by the basename of its Environment directory (immediate non-hidden child of the Environment); **Mirror** materializes it onto Host Volume `internals/workloads/<basename>/` regardless of Source, with durable bytes under `data/workloads/<basename>/` when needed; consumers read known siblings selectively. Canonical Service Network hostname is that basename (except the reserved dial name `database`). Does not gather peers or fulfill other Workloads’ Declarations; not Fabric; not a Component; never installed during Initial Host Provisioning. **Workload** is canonical in docs and code; **workflow** is an accepted conversational alias.
...
**Artifact**:
The portable subset of a **Workload** shape: **Provides** + **Requires** + content (directories, units, route fragments). Missing Propraetor-internal **Workload Manifest** and **Binding**. For `Source` zip the zip root is the Artifact and the Environment Workload directory is Manifest + Binding only (`provides.json` / `requires.json` there fail closed); for `internal` the Artifact lives inline under the Environment Workload directory beside Manifest and Binding.
...
**Route**:
A Workload-offered **Declaration** for Edge: an Edge config fragment in location context (not a full TLS `server`) listed under **Provides** `routes` and attached to a Domain front only via **Binding**. Edge Component Setup gathers Intent-**run** bound Routes and fulfills them into Edge-owned storage for Domain fronts to include; Intent **stop** / **trash** (or missing Binding/Provides) means Edge does not fulfill that Workload’s Routes. Every Provides route path must appear in at least one Binding FQDN array or fulfillment fails closed; Binding FQDNs must be on the Domain want-list. Zero Provides routes is valid. Not projected from the Manifest, not a hostname claim field, and not authored SoT under Workload-tree `routes/` with FQDN-as-filename. Edge ACME does not generate or mutate Routes; Domain fronts remain Edge-owned; Workload Setup does not write Edge interior.
