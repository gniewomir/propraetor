...
**Artifact**:
The portable subset of a **Workload** shape: **Provides** + **Requires** + content (directories, units, route fragments), and optional **Artifact Build** declaration (`build.json`). Missing Propraetor-internal **Workload Manifest** and **Binding**. For `Source` zip or `git` the Artifact is the obtained root (zip after peel when applicable; git tree at the declared `path`); the Environment Workload directory is Manifest + Binding (plus the `.zip` when obtain is a path) — `provides.json` / `requires.json` there fail closed. For `internal` the Artifact lives inline under the Environment Workload directory beside Manifest and Binding. For `local` the operator stages the Projects-root-relative package onto the Host as the Artifact root (unpinned).
...
**Artifact Build**:
Optional Host materialize step declared only by `build.json` at the Artifact root: Propraetor runs the declared container (`image`, `command`) and overlays `output` paths into that root. Missing file means no build; present means validate and run fail-closed. Propraetor does not read other recipe files (e.g. `package.json`). Distinct from Ensure Quadlet `.build` (runtime images under Intent).
_Avoid_: Ensure build, `.build` unit, npm build (when you mean this Propraetor step); Source (when you mean obtain)

**Projects root**:
Operator Configuration directory that roots Manifest **Source** `local` relative paths (`PROPRAETOR_PROJECTS_ROOT`). Absolute or `~/…`; required only when a Workload uses `local` Source.
_Avoid_: Environments root; Environment Configuration; repo root (when you mean this workstation projects tree)
...
**Source**:
The Workload Manifest’s required origin of the Artifact — a discriminated **kind**: `internal` (Artifact is the Environment Workload directory); zip (Artifact is zip root after obtain, peeled to a sole top-level directory when that directory contains Provides — relative `.zip` path under that Workload directory or unauthenticated http(s) zip URI); `git` (pinned https `url` + full-sha `commit` + `path` as Artifact root inside the repo; Environment tree is Manifest + Binding only; Host HTTPS fetch); `local` (path relative to **Projects root**, unpinned; operator stages onto Host; Host never reads the workstation path). Zip path obtain names a regular file, not a symlink. A Workload directory may itself be a symlink across Environments; no symlink under a Workload directory may resolve outside it (operator and Host). No omit. Private / SSH git remotes are not v1.
_Avoid_: package URL, `file://`, public URI (when you mean unauthenticated http(s) zip); branch (when you mean git commit pin); absolute local path in Manifest (use Projects-root-relative)
