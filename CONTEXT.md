...
Operator-machine inputs Propraetor tooling requires (paths and similar), distinct from provider API auth and from Environment Configuration. Today: the matched SSH public- and private-key paths used for Host login and Initial Host Provisioning; the ACME contact email used when an Environment has committed ACME configuration; an optional path to the **Environments root** (directory of `<slug>/` Environment trees) that relocates Environment SoT away from the repository default when set; and an optional path to the **Projects root** (directory that roots Manifest **Source** `local` relative paths for **Artifact Prep**). Baseline may come from the same repo-root gitignored `.env` as Provider Credential; a non-empty process-environment value overrides the file. Path values are absolute or `~/…` only. The ACME contact is optional at load and required only when staging ACME for an Environment that has `acme.json`. The Environments root is optional at load; when set it fully replaces the repo Environments root for tooling (no fallback), and Acceptance / Lifecycle refuse to run with it set. The Projects root is optional at load and required only when a Workload **Source** is `local` (missing then fails closed at Prep). Committed `.env.example` documents the allowlist; neither bag is Environment Configuration.
_Avoid_: Provider Credential, Credential, Environment Configuration, SSH identity / identity (when you mean Adopt or resource identity); SSH key (when you mean a provider-account registry resource); ACME configuration (when you mean the contact email alone); Environment (when you mean the on-disk tree root); Projects root (when you mean Environments root)

**Environment Configuration**:
The Environment-scoped bag of non-committed key/value pairs the operator supplies for Workload Setup to materialize into one Platform User–local EnvironmentFile per Workload at `~/.config/platform/workloads/<basename>/environment`, wired onto that Workload’s `systemd/*.container` install units via a Setup-owned Quadlet drop-in (`EnvironmentFile=` — not unit-text substitution, not onto the Host Volume Workload trees). Baseline from gitignored `environments/<slug>/.env` when present; optional `.env.override` overlays on key collision; current shell overriding any key from either file (precedence: shell > `.env.override` > `.env`); a Workload’s **Binding** remaps bag keys onto names from **Requires** `environment` (surplus bag keys ignored for that Workload); missing remapped keys fail closed in Workload Setup; a non-empty Requires environment map with no `systemd/*.container` also fails closed. For **`internal`** Workloads, the Environment tree may carry inline Artifact contracts beside Manifest and Binding; **Artifact Prep** zips them into **Artifact cache**. For **`git`**, **`local`**, and **`zip`** Sources, the Environment tree is Manifest + Binding only — inline `provides.json` / `requires.json` fail closed (operator Mirror and Setup, and Host materialize before extract); operator remap is Binding + bag only; Host full-fulfills Binding against Artifact Requires and applies the containers gate after materialize. Re-Setup rewrites the file from current sources (the rotation path). Components do not consume this Workload bag. Key names are operator-owned — Setup does not reserve or reject Workload bag names — but prefer not to use the `PLATFORM_*` prefix, **Provider Credential** names (`DIGITALOCEAN_TOKEN` today; future tokens in the same Provider Credential role), **Database admin credentials** (`ROOT_DB_USER`, `ROOT_DB_PASSWORD`), **Cache admin credentials** (`ROOT_CACHE_USER`, `ROOT_CACHE_PASSWORD`), or **Identity admin credentials** (`ROOT_IDENTITY_API_KEY`, `ROOT_IDENTITY_ENCRYPTION_KEY`, `ROOT_IDENTITY_ADMIN_EMAIL`) as Binding remaps into Workloads; those admin credential names are reserved and must not be injected into Workloads (fail closed if remapped). The same `.env` / `.env.override` files may also hold Database, Cache, or Identity admin credentials — they are not this bag. Distinct from Stack/Terraform configuration under that Environment directory, from the process environment as a concept, from **Provider Credential**, from **Operator Configuration**, from **Database admin credentials**, from **Cache admin credentials**, and from **Identity admin credentials**.
...
**Mirror**:
The idempotent materialization of each Environment **Workload** onto Host Volume `workloads/<basename>/` — upsert Manifest + Binding from the Environment tree, ship the staged zip named by **Artifact staging**, extract on the Host, **land the Artifact root** onto that Host Workload tree, and apply **Provides** `directories` (fail closed if reserved Artifact/Propraetor files already collide at the destination, including reserved **Persist**). The staged zip remains on the Host tree as Environment bag. Does not evaluate Manifest **Source** for obtain. Preserves existing Host **Persist** under that owner. For non-`internal` Sources, Environment Workload trees must not contain inline `provides.json` or `requires.json`; **`internal`** Workloads may author Artifact contracts inline for **Artifact Prep** only. Discovers Workloads as every immediate non-hidden directory under the Environment (directory basename = identity; ADR-0033). Does not apply Intent, fulfill Component interior, ship Fabric/Component source, materialize Environment Configuration, obtain materials, or build Artifacts. Updates trees that already exist on the Host, adds missing ones, and **leaves orphans alone**. Orphan removal is **Orphan Reap**.
_Avoid_: HSoT, Host SoT, sync (bare), rsync, Workload Setup, Fabric, Component Setup, Deploy, Orphan Reap (when you mean only this materialize); Source obtain on Host (retired); Purge (retired)

**Artifact Prep**:
The operator-machine, per-Environment step that evaluates committed Manifest **Source** for every Workload, obtains or builds **materials**, optionally runs **Artifact Build**, normalizes to a Propraetor Artifact zip (extract→validate→re-zip for `zip` Source; zip Artifact root for tree Sources), and stages the result in **Artifact cache** with **Artifact staging**. Invoked as Deploy phase 0 and as a standalone entrypoint. Re-runs the full obtain/build pipeline every time; does not rewrite Manifest **Source**. Fails closed when obtain/build fails. Distinct from Mirror, Workload Setup, and Host delivery.
_Avoid_: Mirror, Deploy (when you mean the Host ladder); Host obtain, Host build; Manifest rewrite; skip-build cache hit

**Artifact cache**:
Environment-scoped operator store at `environments/<slug>/.artifact-cache/` holding content-addressed Artifact zips named `<basename>-<sha256>.zip` (SHA-256 hex of zip bytes). Written by **Artifact Prep**; read by Deploy / Mirror via **Artifact staging**. Gitignored; not Host SoT. Older checksum-named zips may accumulate; only the staging record selects the deploy target.
_Avoid_: Host Volume, Persist, Environment Configuration; Manifest Source (when you mean prep input, not this store)

**Artifact staging**:
Per-Workload operator record at `environments/<slug>/.artifact-cache/<basename>.staging`: a single line of SHA-256 hex (64 chars) from the latest **Artifact Prep** run. Zip path is derived as `<basename>-<sha256>.zip`. Deploy / Mirror / singular ensure consume only this staged zip; missing staging record or missing referenced zip fails closed. Distinct from committed Manifest **Source**.
_Avoid_: Manifest Source, Host SoT; zip path in Manifest (Prep does not rewrite Source); JSON staging (v1 is checksum-only)

**Deploy**:
The operator operation that takes a **Substrate** Host to **Deployed**. Always runs **Artifact Prep** as phase 0 for that Environment before Host Workload materialization (standalone Prep entrypoint also exists for iteration). Ladder after Prep: **Fabric Setup** → **Mirror** → **Orphan Reap** → **Component Setup** (`pre-workloads`) → **Workload Setup** (every Environment Workload) → **Component Setup** (`post-workloads`). Deploy consumes staged Artifacts only. No Mirror-or-below Workload materialization without successful Prep and valid **Artifact staging** for every Workload.
_Avoid_: Apply (Stack), ensure (bare), provision, ship; Artifact Prep (when you mean only the Host ladder)
...
An operator-provided Host tree that combines an **Artifact** (zip-delivered) with Propraetor-internal **Workload Manifest** and **Binding**, declares **Intent**, and may author **Declarations** for Components. Identified by the basename of its Environment directory (immediate non-hidden child of the Environment); **Mirror** materializes it onto Host Volume `workloads/<basename>/`, with optional nested **Persist**; consumers read known siblings selectively. Canonical Service Network hostname is that basename (except the reserved dial names `database`, `cache`, and `identity`). Does not gather peers or fulfill other Workloads’ Declarations; not Fabric; not a Component; never installed during Initial Host Provisioning. **Workload** is canonical in docs and code; **workflow** is an accepted conversational alias.
...
**Artifact**:
The portable subset of a **Workload** shape: **Provides** + **Requires** + content (directories, units, route fragments). Missing Propraetor-internal **Workload Manifest** and **Binding**. Produced by **Artifact Prep**, staged in **Artifact cache**, delivered to the Host as a zip via **Mirror**. After Mirror, Host Volume `workloads/<basename>/` holds the landed Artifact root (zip extract + peel when applicable).
_Avoid_: Package, bundle, Workload (when you mean the Environment-placed whole); materials (when you mean the landed Host tree)

**Materials**:
The obtain tree **Artifact Prep** uses before zipping — may be wider than the **Artifact** root so **Artifact Build** can resolve monorepo / `file:` / workspace paths. Prep-only; never landed on the Host. Discarded after the staged zip is written.
_Avoid_: Artifact cache, Host Workload tree, Persist; Source (when you mean the Manifest declaration)

**Artifact Build**:
Optional **Artifact Prep** step declared only by `build.json` at the Artifact root inside **materials**: Prep runs the declared container on the operator with materials mounted and cwd at the Artifact root, then overlays `output` paths before zipping. Missing file means no build. Distinct from Ensure Quadlet `.build` (runtime images under Intent). Not a Host step.
_Avoid_: Ensure build, `.build` unit, Host materialize build; npm build (when you mean project tooling outside Propraetor)

**Projects root**:
Operator Configuration directory that roots Manifest **Source** `local` relative Artifact `path` values (`PROPRAETOR_PROJECTS_ROOT`) for **Artifact Prep** — the parent of one or more projects, not **materials** and not a single **Project root**. Absolute or `~/…`; required only when a Workload **Source** is `local`.
_Avoid_: Environments root; Environment Configuration; Project root; Artifact cache; Host Volume

**Project root**:
The `git` toplevel of the repository that contains a `local` **Source** Artifact path. **Artifact Prep** uses this tree as **materials** (fail closed when toplevel cannot be established or escapes **Projects root**). Distinct from **Projects root**.
_Avoid_: Projects root; materials (when you mean the whole obtain tree); Artifact

**Provides**:
A Workload **Declaration** of what the Artifact offers: open map of directories to materialize onto the Host Workload tree; route fragment paths offered for Edge (path → human description); optional permission catalog for Identity (key → human description); optional `oidc_callback` path (value is the path, `/…` only). Declared ⇒ used; omit ⇒ absent — no `false`. Non-empty `permissions` means this Workload contributes permission keys into the Environment-scoped Identity audience; those keys must be unique across Workloads in that Environment. Invalid permission-key shapes, missing mandatory marker keys, empty catalog/client permission sets, and duplicate API permission keys are rejected fail-closed during Workload Setup. For API-level access control, the operator must include the mandatory per-API marker permission key `${workload-slug}:api` in this Workload’s `permissions` map, and the API authorization layer must require that marker key. Propraetor only constrains issued tokens by requested scope keys; API-side authz must enforce the marker key. All other permission keys in this Workload’s `permissions` map must follow the convention `${workload-slug}:${permission}` (where `${permission}` is the operator-chosen capability identifier). Route fragments are a separate channel into Edge-owned storage, not Workload-tree Route SoT. Descriptions are human-only except `oidc_callback`, whose value is the path.
...
**Source**:
The Workload Manifest’s required prep-facing declaration of where **materials** or a finished Artifact come from — evaluated only by **Artifact Prep**, never rewritten by Prep, not used by Mirror for Host obtain. Authoring shape: ADR-0060. Discriminated kinds (v1): `internal` (`{ "kind": "internal" }` — Environment Workload directory authors the whole Workload; **materials** = that directory excluding `manifest.json` and `binding.json`; inline Artifact contracts allowed; optional **`build.json`** at Artifact root); `zip` (`{ "kind": "zip", "path" }` or `{ "kind": "zip", "uri" }` — finished Artifact zip; Prep extract→validate→re-zip); `git` (pinned https `url` + full-sha `commit`; **materials** = repo tree; Artifact root = `path` inside it); `local` (Manifest `path` under **Projects root**; **materials** = **Project root**). Optional **`build.json`** on tree Sources triggers **Artifact Build** during Prep. Host delivery is always the staged zip from **Artifact cache** via **Artifact staging**.
_Avoid_: Host obtain; Manifest rewrite; symlink; absolute local path in Manifest (use Projects-root-relative for `local`); checksum as Source pin; blanket Manifest + Binding only ( **`internal`** is the exception); string `"internal"`; bare zip path or URI string as Source
