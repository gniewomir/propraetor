...
**Artifact**:
The portable subset of a **Workload** shape: **Provides** + **Requires** + content (directories, units, route fragments). Missing Propraetor-internal **Workload Manifest** and **Binding**. For `Source` zip the zip root is the Artifact; for `internal` the Artifact lives inline under the Environment Workload directory beside Manifest and Binding.
