...
Operator-machine inputs Propraetor tooling requires (paths and similar), distinct from provider API auth and from Environment Configuration. Today: the matched SSH public- and private-key paths used for Host login and Initial Host Provisioning; the ACME contact email used when an Environment has committed ACME configuration; an optional path to the **Environments root** (directory of `<slug>/` Environment trees) that relocates Environment SoT away from the repository default when set; and an optional path to the **Projects root** (directory that roots Manifest **Source** `local` relative paths). Baseline may come from the same repo-root gitignored `.env` as Provider Credential; a non-empty process-environment value overrides the file. Path values are absolute or `~/…` only. The ACME contact is optional at load and required only when staging ACME for an Environment that has `acme.json`. The Environments root is optional at load; when set it fully replaces the repo Environments root for tooling (no fallback), and Acceptance / Lifecycle refuse to run with it set. The Projects root is optional at load and required only when a Workload Source is `local` (missing then fails closed). Committed `.env.example` documents the allowlist; neither bag is Environment Configuration.
...
**Artifact**:
The portable subset of a **Workload** shape: **Provides** + **Requires** + content (directories, units, route fragments), and optional **Artifact Build** declaration (`build.json`). Missing Propraetor-internal **Workload Manifest** and **Binding**. May sit at a `path` inside wider **materials** during obtain/build; after materialize, Host Volume `workloads/<basename>/` holds only this Artifact root. For `Source` zip the Artifact is zip root after peel when applicable. For `git` / `local` the Artifact is the `path` inside materials (repo at commit, or staged **Project root**; for `local`, Host sees the Project-root-relative path after operator stage rewrite). For `internal` the Artifact lives inline under the Environment Workload directory beside Manifest and Binding. Zip/`git` Environment trees must not contain `provides.json` / `requires.json`.
_Avoid_: Package, bundle, Workload (when you mean the Environment-placed whole); zip (when you mean the concept, not the Source form); materials (when you mean the landed Host tree)

**Materials**:
The obtain tree that may be wider than the **Artifact** root so **Artifact Build** can resolve monorepo / `file:` / workspace paths. For `git`, the repository at the pinned commit; for `local`, the staged **Project root**; for `internal` / zip, equal to the Artifact. Discarded after the Artifact is landed on the Host Workload tree (v1).
_Avoid_: Artifact, Projects root, Project root (when you mean the Host Workload tree); Persist

**Artifact Build**:
Optional Host materialize step declared only by `build.json` at the Artifact root: Propraetor runs the declared container (`image`, `command`) with **materials** mounted and cwd at the Artifact root, then overlays `output` paths into that root before landing. Missing file means no build; present means validate and run fail-closed. Propraetor does not read other recipe files (e.g. `package.json`). Distinct from Ensure Quadlet `.build` (runtime images under Intent).
_Avoid_: Ensure build, `.build` unit, npm build (when you mean this Propraetor step); Source (when you mean obtain)

**Projects root**:
Operator Configuration directory that roots Manifest **Source** `local` relative Artifact `path` values (`PROPRAETOR_PROJECTS_ROOT`) — the parent of one or more projects, not materials and not a single **Project root**. Absolute or `~/…`; required only when a Workload uses `local` Source.
_Avoid_: Environments root; Environment Configuration; Project root; repo root (when you mean one git checkout); Artifact; materials

**Project root**:
The `git` toplevel of the repository that contains a `local` Source Artifact path. Operator stages this tree as **materials** for `local` (fail closed when toplevel cannot be established or escapes **Projects root**). Distinct from **Projects root**.
_Avoid_: Projects root; materials (when you mean the workstation checkout); Artifact
...
**Source**:
The Workload Manifest’s required origin of the Artifact — a discriminated **kind**: `internal` (Artifact is the Environment Workload directory); zip (Artifact is zip root after obtain, peeled to a sole top-level directory when that directory contains Provides — relative `.zip` path under that Workload directory or unauthenticated http(s) zip URI); `git` (pinned https `url` + full-sha `commit`; **materials** = repo tree; Artifact root = `path` inside it; Environment tree is Manifest + Binding only; Host HTTPS fetch); `local` (Manifest `path` is Artifact under **Projects root**; **materials** = staged **Project root**; operator rewrites staged Manifest `path` to Project-root-relative; unpinned; Host never reads the workstation path). Zip path obtain names a regular file, not a symlink. A Workload directory may itself be a symlink across Environments; no symlink under a Workload directory may resolve outside it (operator and Host). No omit. Private / SSH git remotes are not v1.
_Avoid_: package URL, `file://`, public URI (when you mean unauthenticated http(s) zip); branch (when you mean git commit pin); absolute local path in Manifest (use Projects-root-relative); Projects root as materials; staging only the Artifact path when monorepo siblings under the Project root are required for build
