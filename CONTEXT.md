...
**Source**:
The Workload Manifest’s required prep-facing declaration of where **materials** or a finished Artifact come from — evaluated only by **Artifact Prep**, never rewritten by Prep, not used by Mirror for Host obtain. Discriminated kinds (v1): `zip` (finished Artifact zip — relative path under the Workload directory or unauthenticated http(s) URI; Prep extract→validate→re-zip); `git` (pinned https `url` + full-sha `commit`; **materials** = repo tree; Artifact root = `path` inside it); `local` (Manifest `path` under **Projects root**; **materials** = **Project root**); `internal` (string `"internal"` — the Environment Workload directory authors the whole Workload; **materials** = that directory excluding `manifest.json` and `binding.json`; inline Artifact contracts allowed; optional **`build.json`** at Artifact root). Optional **`build.json`** on tree Sources triggers **Artifact Build** during Prep. Host delivery is always the staged zip from **Artifact cache** via **Artifact staging**.
_Avoid_: Host obtain; Manifest rewrite; symlink; absolute local path in Manifest (use Projects-root-relative for `local`); checksum as Source pin; blanket Manifest + Binding only ( **`internal`** is the exception)
