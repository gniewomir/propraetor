...
An operator-provided Host tree that combines an **Artifact** with Propraetor-internal **Workload Manifest** and **Binding**, declares **Intent**, and may author **Declarations** for Components. Identified by the basename of its Environment directory (immediate non-hidden child of the Environment); **Mirror** materializes it onto Host Volume `workloads/<basename>/` regardless of Source, with optional nested **Persist**; consumers read known siblings selectively. Canonical Service Network hostname is that basename (except the reserved dial names `database` and `cache`). Does not gather peers or fulfill other Workloads’ Declarations; not Fabric; not a Component; never installed during Initial Host Provisioning. **Workload** is canonical in docs and code; **workflow** is an accepted conversational alias.
...
**Requires**:
A Workload **Declaration** of what the Artifact needs from Propraetor: environment variable names the process expects (name → human description) and required booleans `database` and `cache`. Descriptions are human-only. Binding remaps Environment Configuration bag keys onto those names; Database gathers `database: true` and Cache gathers `cache: true` when Intent is **run**.
...
**Orphan Reap**:
The sole operation that permanently removes a Workload’s Host state when that basename is not present in the Environment’s discovered Workload set (no immediate non-hidden directory under `environments/<slug>/`), including that Workload’s Host Volume owner tree (SoT and **Persist**), Platform User units, and EnvironmentFile trees (Database fulfillment for that basename is dropped on the next Component Setup `post-workloads`). Distinct from **Mirror** (which leaves orphans alone). Not an Intent.
