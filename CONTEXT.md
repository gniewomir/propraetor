...
**Identity**:
The public Identity Provider Component on a public Host (idle allowed with zero claimants). Engine is Pocket ID. Gathers **Requires** `identity: true` with a permission catalog (**Provides** `permissions`, Bound to exactly one FQDN) and/or an OIDC client (**Requires** `permissions` plus **Provides** `oidc_callback`). Fulfills catalog, client registration, and client grants; publishes issuer, JWKS URL, and audience to a resource-server Workload and issuer, client id (basename), callback URLs, resource, and scopes to a client Workload. Intent **stop** unpublishes that binding and **leaves** Identity interior records; omitting the Declaration or **Orphan Reap** drops them. Distinct from **Edge** (Edge is not the token verifier) and from the **Service Network** (no tokens between co-tenant Workloads). Issuer hostname is an Environment-authored Declaration on the Domain want-list.
