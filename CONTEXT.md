...
**Projects root**:
Operator Configuration directory that is **materials** for Manifest **Source** `local` and roots the relative Artifact `path` (`PROPRAETOR_PROJECTS_ROOT`). Absolute or `~/…`; required only when a Workload uses `local` Source. For monorepo packages, set this to the workspace root.
_Avoid_: Environments root; Environment Configuration; repo root (when you mean this workstation projects tree); Artifact
...
**Source**:
The Workload Manifest’s required origin of the Artifact — a discriminated **kind**: `internal` (Artifact is the Environment Workload directory); zip (Artifact is zip root after obtain, peeled to a sole top-level directory when that directory contains Provides — relative `.zip` path under that Workload directory or unauthenticated http(s) zip URI); `git` (pinned https `url` + full-sha `commit`; **materials** = repo tree; Artifact root = `path` inside it; Environment tree is Manifest + Binding only; Host HTTPS fetch); `local` (**materials** = staged **Projects root**; Artifact root = relative `path` inside it; unpinned; Host never reads the workstation path). Zip path obtain names a regular file, not a symlink. A Workload directory may itself be a symlink across Environments; no symlink under a Workload directory may resolve outside it (operator and Host). No omit. Private / SSH git remotes are not v1.
_Avoid_: package URL, `file://`, public URI (when you mean unauthenticated http(s) zip); branch (when you mean git commit pin); absolute local path in Manifest (use Projects-root-relative); staging only the package path when materials must be the workspace root
