...
**Identity**:
The public Identity Provider Component on a public Host (idle allowed with zero claimants). Engine is Pocket ID. Service Network dial name `identity` (Workload basename `identity` is rejected). Issuer hostname declared in committed `environments/<slug>/identity.json` (one want-list FQDN; missing file = no issuer, Identity idles); Edge owns the Domain front for that FQDN with an Edge-owned proxy block to the Identity dial name — not a Workload Route; Edge fails closed if any Workload Binding attaches Routes to the Identity-claimed FQDN. Gathers **Requires** `identity: true` with a permission catalog (**Provides** `permissions`, Bound to exactly one FQDN) and/or an OIDC client (**Requires** `permissions` plus **Provides** `oidc_callback`). Fulfills catalog, client registration, and client grants; publishes issuer, JWKS URL, and audience to a resource-server Workload and issuer, client id (basename), callback URLs, resource, and scopes to a client Workload. Intent **stop** unpublishes that binding and **leaves** Identity interior records; omitting the Declaration or **Orphan Reap** drops them. Distinct from **Edge** (Edge is not the token verifier) and from the **Service Network** (no tokens between co-tenant Workloads). Changing the issuer hostname is a WebAuthn RP ID change (every passkey breaks) — treat as effectively immutable.
...
Committed Environment file `environments/<slug>/identity.json` declaring which single Domain want-list FQDN the Identity Component binds to as its issuer hostname. Missing file means no Identity issuer for that Environment (Identity idles). Fail closed if the FQDN is not on the want-list. `ensure-components` stages it onto the Host alongside the ACME want-list and admin credentials. Peer of `domains.json` and `acme.json` — an Environment-authored Declaration input. Changing the issuer FQDN is a WebAuthn RP ID change (every passkey breaks).
