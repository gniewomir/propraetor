...
**Acceptance:** Suite baseline between cases is **Deployed**. The runner re-converges via **Deploy** before each case (failure artifacts remain until the next baseline). Cases restore Environment SoT / Intent to committed truth before exit; the runner owns Host convergence. After each baseline Deploy, the runner snapshots `environments/<slug>/` (excluding `.ssh/` Host-session TOFU) and asserts the tree is identical after the case — covering gitignored files (`.env`, `.env.override`) and leftover fixtures; a dirty tree fails the slot. Host Volume Persist / owner trees: no destruction except as expressed operator Intent (Environment absence → Orphan Reap via Deploy is fine); cases own cleanup only of case-created durable residue that would survive the next Deploy; runner **G** is tracked-path only (v1); full preexisting Persist oracle deferred. Non-**test** Acceptance is diagnostic: typed `diagnose <slug>`; Environment SoT for the active slug stays at committed `HEAD` (no fixtures / SoT mutation — enforced, not warn-only); fixture-class cases (those that track Environment fixtures or SoT restores) are skipped on a full run and refused when explicitly selected; baseline Deploy aborts if that Environment tree is dirty vs `HEAD`; case-owned Host Volume Persist probes remain allowed; Deploy still runs. No Deployed assert-G in v1. No semantic “undeclared Host data loss” oracle — the checkable proxy is Environment SoT immutability off **test**.
...
**Amended by ADR-0054 / #217:** Host Volume destroy path is Environment absence → Orphan Reap only (no Intent trash / Purge).
