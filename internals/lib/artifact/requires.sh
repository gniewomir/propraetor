#!/usr/bin/env bash
# Requires Declaration contract (ADR-0053 / #199).
#
# artifact_requires_validate PATH
#   Fail closed on invalid Requires JSON (required boolean database;
...
extra = sorted(set(raw) - {"environment", "database"})
if extra:
    raise SystemExit(
        f"Requires allows only environment/database in {path}; unexpected: {', '.join(extra)}"
    )
