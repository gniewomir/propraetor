...
**Entrypoint:** `./test.sh acceptance` — see [docs/agents/testing.md](../../../docs/agents/testing.md) (ADR-0036 / ADR-0056). Suite baselines: [ADR-0042](../../../docs/adr/0042-suite-baselines-and-test-isolation.md).
...
- Cases restore Environment SoT / Intent to committed truth before exit (remove fixtures). Runner owns Host convergence.
...
- **Host Volume Persist / owner trees:** do not destroy bytes except as expressed operator Intent (Environment absence → Orphan Reap via Deploy is fine). Case-owned cleanup only for case-created durable residue that would **survive** the next Deploy; register those paths for tracked **G**. No peer-pollution cleanup.
- **`test` only** for Environment fixtures / SoT mutation (track helpers, Orphan Reap fixture paths).
- Non-**test**: type exact `diagnose <slug>` (slug matches `--env`); Environment SoT stays at committed `HEAD`; fixture-class cases skipped (full suite and `--from` remainder, including a named start) or refused (explicit selector); baseline Deploy aborts if `environments/<slug>/` is dirty vs `HEAD`; case-owned Host Volume `data/` probes still allowed; Deploy still runs.
...
./test.sh acceptance 1100            # one slice (numeric prefix; 1100 == 1100-podman)
./test.sh acceptance podman          # one slice (unique filename substring)
./test.sh acceptance --from 2000     # 2000-domain-front-healthcheck through the end
...
./test.sh acceptance 1100-podman --env test
```

**Environment (ADR-0019 / ADR-0042):** no `--env` → **test** (workspace `default`). `--env test` / `--env default` are aliases. Any other slug requires explicit `--env <slug>` and typed `diagnose <slug>`. Positionals first, then flags; flag order free (ADR-0039). Case-selector and `--from` are mutually exclusive (ADR-0056).
...
1. Pick the next prefix: default gap 100 (append last+100; today that is `5600`). Insert with a spare (`0150` between `0100` and `0200`). Prefixes are unique four-digit numbers; C-sort is suite order (ADR-0056).
2. Add `NNNN-short-name.sh` — one capability / contract slice per file. Suite order is fail-fast layering (Host/Durables → Substrate → Edge → Deploy → Workload → Examples → Database → Cache), not runtime dependency (ADR-0042).
...
6. Keep the script focused on external behavior. The runner discovers `[0-9]*.sh` and requires `NNNN-` — no registry edit.
...
| `NNNN-*.sh` | Acceptance Tests (sort order = run order) |
