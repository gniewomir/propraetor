...
# JSON via argv: pipe + `python3 - <<'PY'` loses stdin to the heredoc.
...
# -f is required: without it, GET 404 still exits 0 and create is skipped.
...
python3 - "${RESOURCE}" "${WL}" "${token_json}" <<'PY'
import base64, json, sys

resource, wl, raw = sys.argv[1], sys.argv[2], sys.argv[3]
marker = f"{wl}:api"
start = raw.find("{")
end = raw.rfind("}")
