...
if [[ -n "${ROUTE_FQDN}" ]]; then
  ...
  cat >"${FIX_DIR}/alpha/routes/probe.conf" <<EOF
...
EOF
  acceptance_bind_route_fragment "${FIX_DIR}/alpha" "routes/probe.conf" "${ROUTE_FQDN}"
fi
...
host_ssh \
  "rm -rf /host-volume/workloads/alpha \
          /host-volume/workloads/legacy \
          /host-volume/workloads/named \
          /host-volume/workloads/upstreamed \
          /host-volume/workloads/nosource \
          /host-volume/workloads/sourced \
          /host-volume/workloads/retired-env \
          /host-volume/workloads/retired-db \
          /host-volume/workloads/zero \
...
if [[ -n "${ROUTE_FQDN}" ]]; then
  sot="$(host_ssh \
    "cat /host-volume/workloads/alpha/routes/probe.conf")"
  echo "${sot}" | grep -q 'operator-authored Route fragment for alpha' \
    || fail "Provides Route fragment not stored under workloads/alpha/routes/"
  host_ssh \
    "grep -Fq '${ROUTE_FQDN}' /host-volume/workloads/alpha/binding.json" \
    || fail "Binding must attach the fragment to ${ROUTE_FQDN}"
  edge_before="$(host_ssh \
    "ls /host-volume/components/edge/persist/routes/alpha.conf \
         /host-volume/components/edge/persist/routes/alpha--* 2>/dev/null || true")"
  [[ -z "${edge_before}" ]] \
  ...
  installed="$(host_ssh \
    "cat /host-volume/components/edge/persist/routes/alpha--${ROUTE_FQDN}.conf")"
  [[ "${installed}" == "${sot}" ]] || fail "installed Route bytes must match Provides fragment"
  echo "${installed}" | grep -q 'Generated by Propraetor' \
    && fail "Propraetor must not generate Route shells" || true
  pass "Edge Setup gathers Binding-attached Provides fragment; thin Manifest; no invented Quadlet"
else
...
host_ssh "test -f /host-volume/workloads/zero/manifest.json" \
